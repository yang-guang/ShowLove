...
import cn.yg.util.Font16;
import cn.yg.util.Font24;
import cn.yg.util.Font32;
import cn.yg.util.Util;

public class FifthSurfaceView extends SurfaceView implements SurfaceHolder.Callback{
	...
	public FifthSurfaceView(Context context, int s_w, int s_h, Handler handler) {
		super(context);
	...
	class showheart extends Thread {
		...
		public void run() {
			...
			FifthSurfaceView.this.show_I(istartx,istarty, 7, 9,
					"array_I.txt", 18, 1);
			run_hua_heart();
			FifthSurfaceView.this.show_I(ustartx,ustarty,
	...
	//鍦ㄥ啓鏂囦欢杩欎釜锛屾垜鏈�涓昏杩樻槸鍠滄鐢盇WT杞寲鑰屽緱鍒扮殑鏁扮粍锛屽彲鏄痑wt鏃犳硶绉绘鍒癮ndroid閲岋紝浜庢槸浣跨敤鐨勭偣闃靛瓧搴�
	//鐐归樀瀛楀簱鍒�16*16锛�24*24锛�32*32锛�48*46锛屾瘡绉嶅張鍒唖,k,h,f(妤峰畫榛戝晩浠�涔堢殑),浠ｇ爜閲屾湁涓夌鐨勬柟娉曪紝鑷充簬瀛楃搴撳彲浠ヨ嚜宸变笅杞戒篃鍙�
	//鎵炬垜瑕侊紝鎴戞妸姣忕閮借瘯杩囷紝鏈�缁堥�夋嫨浜咹K24s
	class showWenZi extends Thread {
		...
		public void run() {
			...
			String one =getResources().getString(R.string.first_et_1);
			...
			String two = getResources().getString(R.string.first_et_2) ;
	...
	class showheart extends Thread {
		...
		private void run_M_M(){
			int start_x = sw/2-100;
			int start_y = sh/2-110;
			Bitmap m_m = bitmapcache.getBitmap(R.drawable.h_m_m, mContext);
			int pic_w = m_m.getWidth();
			int pic_h = m_m.getHeight();
			Bitmap five1=bitmapcache.getBitmap(R.drawable.five1, mContext);
			m_m=zoomImage(five1, pic_w, pic_h);
